...
public class TaskDetailsDaoImpl implements TaskDetailsDao {
	...
	public List<TaskInfo> displayTask(String userId) {
		...
		while(itr.hasNext()){
			...
			queryComments.setParameter("taskid",taskInfo.getTaskDetails().getTaskid());
			List<CommentsDetails> comments = queryComments.list();
			/*Iterator itrcomments = comments.iterator();
			List<CommentsDetails> commentsList = new ArrayList<CommentsDetails>();
			while(itrcomments.hasNext()){
				Object[] commentsObj = (Object[]) itrcomments.next();
				for(int i=0;i<commentsObj.length;i++){
					if(commentsObj[i] instanceof CommentsDetails){
						commentsList.add((CommentsDetails) commentsObj[i]);
					}
				}
			}*/
		...
		/*CommentsDetails cd = results.get(0).getCommentsDetails();
		System.out.println(cd.getComments());*/

		
		/*
		
		Criteria criteria = session.createCriteria(TaskDetails.class);
		criteria.add(Restrictions.eq("userid", userId));
		List<TaskDetails> taskDetails = criteria.list();*/
